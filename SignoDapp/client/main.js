...
Template.CreateNewCertificateForm.events({
    "submit .newCertificateForm": function (event, t) {
        var candidateName = event.target.candidateName.value;
        var candidateDOB = event.target.candidateDOB.value;
        var elements = document.getElementById("newCertificateForm").elements;
        var json = elementToJSON(elements);
        JSONToMap(json);
        var isAnonmyous = event.target.isAnonymous.checked;

        console.log(isAnonmyous);
...
function elementToJSON(elements) {
    ...
    for (var element in elements) {
        if (elements[element].value !== "" && elements[element].value !== undefined && isNaN(parseInt(element, 10)) && element !== "isAnonymous") {
            ...
            var newEntry = JSON.stringify(dictEntry).toString();
